...
> **Lesson**: Logic Circuits (`7-3`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 91–94 (book numbering)
> **Output filename**: `logic-circuits.html`
...
Save the site as `logic-circuits.html` in this same folder.
...
**2. مفهوم Logical operations** — core concept (book pages 91–94). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Logical operations`. Interaction: animated SVG system diagram (no native text) with a step-reveal; replay on each part . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. مفهوم Logic circuit** — core concept (book pages 91–94). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Logic circuit`. Interaction: animated SVG system diagram (no native text) with a step-reveal; replay on each part . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**4. مفهوم Truth table** — core concept (book pages 91–94). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Truth table`. Interaction: animated SVG system diagram (no native text) with a step-reveal; replay on each part . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**5. مفهوم AND gate (logical conjunction circuit)** — core concept (book pages 91–94). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `AND gate (logical conjunction circuit)`. Interaction: animated SVG system diagram (no native text) with a step-reveal; replay on each part . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**6. مفهوم OR gate (logical disjunction circuit)** — core concept (book pages 91–94). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `OR gate (logical disjunction circuit)`. Interaction: animated SVG system diagram (no native text) with a step-reveal; replay on each part . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
...
(1) Name the following logic circuit and create a truth table.
(2) A circuit was created by combining logic circuits as shown in the following diagram. In this case, create
a truth table for inputs A, B, and output X.
Explanation
(1)	 AND Circuit (logical conjunction circuit).
An AND circuit is a circuit that outputs 1 only when all inputs are 1. Therefore, the truth table is as
follows.
Input
Output
A
...
1
1
(2) Create and analyze the truth table for each circuit.
...
Answer the following questions.
(1) Name the following logic circuit and create a truth table.
(2) A circuit was created by combining logic circuits as shown in the following diagram. In this case, create a truth
table for inputs A, B, and output X.
(3) In the following half adder circuit, determine the values of each output a to d when input A is 0 and input B is
1.
(4) In the following full adder circuit, determine the values of each output a to f when input A is 1, input B is 1, and
input Cin is 0.
Try
A
B
...
Answer the following questions.
(1) Name the following logic circuit and create a truth table.
(2) A circuit was created by combining logic circuits as shown in the following diagram. In this case, create a
truth table for inputs A, B, and output X.
(3) In the following half adder circuit, determine the values of each output a to d when input A is 1 and input
B is 1.
(4) In the following full adder circuit, determine the values of each output a to f when input A is 1, input B is
0, and input Cin is 1.
1
A
X
X
