...
> **Lesson**: Numerical Calculations [1] (`6-5`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 65–66 (book numbering)
> **Output filename**: `numerical-calculations-1.html`
...
Save the site as `numerical-calculations-1.html` in this same folder.
...
**2. النظام الثنائي (Binary addition)** — core concept (book pages 65–66). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Binary addition`. Interaction: step-by-step conversion diagram on a multiples-of-10 grid; each click flips a digit; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. النظام الثنائي (Binary subtraction)** — core concept (book pages 65–66). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Binary subtraction`. Interaction: step-by-step conversion diagram on a multiples-of-10 grid; each click flips a digit; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
