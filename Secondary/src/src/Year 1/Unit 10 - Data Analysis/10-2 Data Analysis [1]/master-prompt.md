...
> **Lesson**: Data Analysis [1] (`10-2`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 127–130 (book numbering)
> **Output filename**: `data-analysis-1.html`
...
Save the site as `data-analysis-1.html` in this same folder.
...
**2. مفهوم Frequency distribution table** — core concept (book pages 127–130). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Frequency distribution table`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. مفهوم Histogram** — core concept (book pages 127–130). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Histogram`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**4. مفهوم Mean** — core concept (book pages 127–130). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Mean`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**5. مفهوم Median** — core concept (book pages 127–130). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Median`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**6. مفهوم Mode** — core concept (book pages 127–130). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Mode`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
...
(1)	 The data on the right represents the results of a handball
throw from a class of 32 students. Answer [1] and [2].
[1]	 Create a frequency distribution table where the
class interval is 12 m to less than 15 m, with each
class having a width of 3 m.
[2]	 Based on the frequency distribution table created
in question [1], draw a histogram.
(2)	 The data on the right represents the scores of 12
students who played a game. Determine the mean,
median, and mode of the data.
Explanation
(1)	 [1]
(2)	 Mean:  = 6 + 9 + 3 + 6 + 10 + 12 + 4 + 3 + 8 + 10 + 7 + 6
12
= 84
12 = 7
7 points
Median: First, arrange all the data in ascending order.
3,  3,  4,  6,  6,  6,  7,  8,  9,  10,  10,  12
Since the number of data points is even, the median is the mean of the two central values, which
are 6 and 7.
Therefore,　　6 + 7
2
= 6.5                6.5 points
Mode: The value that occurs most frequently is 6
6 points
15
20
21
...
2
1
12 15 18 21 24 27 30 33 (m)
[2]	 (Persons)
129
The following data represents the quiz scores of 20 high school students. Answer the following questions.
3
4
9
7
...
The following data shows the recorded daily maximum temperatures in Tokyo for February 2023. Answer the
following questions.
13.1
9.2
6.2
...
15
(°C)
(1)	 Create a frequency distribution table where the class interval is 14°C to less than 16°C, with each class
having a width of 2°C.
(2)	 Based on the frequency distribution table created in step (1), draw a histogram.
The following table summarizes the functions available in spreadsheet software. Choose the term that best fits
into the blanks [1] to [3] from the options A to D below, and answer using the letters.
Function
Explanation
(  [1]  )
Determines the mean.
