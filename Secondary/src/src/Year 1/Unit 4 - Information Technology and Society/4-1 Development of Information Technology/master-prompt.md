...
> **Lesson**: Development of Information Technology (`4-1`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 42–45 (book numbering)
> **Output filename**: `development-of-information-technology.html`
...
Save the site as `development-of-information-technology.html` in this same folder.
...
**2. مفهوم Society 5.0** — core concept (book pages 42–45). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Society 5.0`. Interaction: scenario cards with a yes/no disclosure flow + step reveal (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. البيانات (Big Data)** — core concept (book pages 42–45). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Big Data`. Interaction: scenario cards with a yes/no disclosure flow + step reveal (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**4. الذكاء الاصطناعي (AI (Artificial Intelligence))** — core concept (book pages 42–45). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `AI (Artificial Intelligence)`. Interaction: scenario cards with a yes/no disclosure flow + step reveal (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**5. مفهوم Machine Learning** — core concept (book pages 42–45). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Machine Learning`. Interaction: scenario cards with a yes/no disclosure flow + step reveal (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**6. مفهوم IoT** — core concept (book pages 42–45). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `IoT`. Interaction: scenario cards with a yes/no disclosure flow + step reveal (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
...
(1)	 Choose the one that is the most appropriate sequence of societal development from the options A to D
below, and answer using the letter.
A
Hunter-gatherer society → Agrarian society → Industrialized society → Information society
B
Agrarian society → Hunter-gatherer society → Information society → Industrialized society
C
Agrarian society → Hunter-gatherer society → Industrialized society → Information society
D
Hunter-gatherer society → Agrarian society → Information society → Industrialized society
(2)	 Choose the most closely related option for the following sentences from the options 1) to 5) below, and
answer using the letters.
[1]	 Technology that connects all things (objects) to the internet.
[2]	 A technology in which machines analyze data to extract rules and patterns from the data.
[3]	 A vast amount of data that is difficult for humans to fully comprehend.
[4]	 A virtual space which is created by a computer and allows for experiences that feel like reality.
A  Machine Learning
B  Big data
C  IoT (Internet of Things)
D  Virtual Reality
E  Augmented Reality
(3)	 What is the term for the gap between those who can utilize information technology, such as the internet,
and those who cannot?
Explanation
(1)	 A
(2)	 [1]  C    [2]  A    [3]  B    [4]  D
(3)	 Digital Divide
Changes in Life Caused by Information Technology
(1)
(17Technostress) : The overall term for mental and physical problems that arise from using
computers.
•	 (18Technophobia) : A state of anxiety or fear towards using computers due to a sense of discomfort
or lack of confidence in handling them.
•	 (19Techno-Addiction) : Excessive dependency on computers.
•	 (20VDT Disorder) : Shoulder stiffness and eye strain caused by prolonged computer use.
(2)
(21Digital Divide) : The gap between those who can utilize information technology and those who
cannot.
3
Warm Up
44
Read the following passage and answer the questions.
In ancient times, people lived by hunting animals and gathering plants. In such a (  [1]  ) society,
technologies like stone axes, bows and arrows, fire-starting techniques, and even language itself supported the
community. Eventually, humanity settled on fertile land and led a life growing crops. In this kind of (  [2]  )
society, the community was supported by technologies such as the invention of writing, agricultural tools,
astronomy
```
