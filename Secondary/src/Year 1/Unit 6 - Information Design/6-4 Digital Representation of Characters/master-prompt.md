...
> **Lesson**: Digital Representation of Characters (`6-4`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 61–64 (book numbering)
> **Output filename**: `digital-representation-of-characters.html`
...
Save the site as `digital-representation-of-characters.html` in this same folder.
...
**2. مفهوم Character code** — core concept (book pages 61–64). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Character code`. Interaction: visual card per concept + step-reveal worked example (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. مفهوم Character code system** — core concept (book pages 61–64). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Character code system`. Interaction: visual card per concept + step-reveal worked example (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**4. مفهوم Encoding** — core concept (book pages 61–64). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Encoding`. Interaction: visual card per concept + step-reveal worked example (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**5. مفهوم Character corruption** — core concept (book pages 61–64). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Character corruption`. Interaction: visual card per concept + step-reveal worked example (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**6. مفهوم Font** — core concept (book pages 61–64). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Font`. Interaction: visual card per concept + step-reveal worked example (replay) . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
...
(1)	 Choose the numbers or terms that best fit into the blanks [1] to [5] from the options A to H below.
Characters and symbols can also be represented by combinations of 0s and 1s. A system that maps
characters and symbols to binary or hexadecimal is called a (  [1]  ). There are several types within
the ([1]), including (  [2]  ), which accommodates the writing systems of countries around the world.
Alphanumeric characters and symbols are represented by (  [3]  ) byte, 1 byte can represent
...
D	 256
E	 Character code
F	 Character corruption
G	 Unicode
H	 ASCII code
(2)	 Use the character code table on the
right to answer the following
questions.
[1]	 Use binary to represent the
character code for “M”.
[2]	 Use hexadecimal to represent
the
character
code
for
“MILK”.
[3]	 What is the character string
corresponding to the character
code “4C6F7665(16)”?
Explanation
(1)	 [1]  E    [2]  G    [3]  A    [4]  B    [5]  D
(2)	 [1]	 From the character code table, the most significant 4 bits of “M” are 0100 and the least significant 4
bits are 1101; therefore, 01001101(2)
[2]	 According to the character code table, the character codes corresponding to “M, I, L, K” are 4D, 49,
4C, and 4B, respectively, in the order of most significant to least significant. Connecting these
character codes results in 4D494C4B(16)
[3]	 Consider the character code as separated into “4C/6F/76/65”.
“4C” corresponds to the character “L” because it is located at the intersection of the most
significant 4 and the least significant C.
Similarly, “6F” corresponds to “o”, “76” to “v”, and “65” is “e”, so the resulting character string is
Love
...
0000
0
Symbols for Controlling Computers
(Omission)
SP
0
@
...
63
Answer the following questions.
(1)	 Complete the following sentences by filling in the blanks [1] to [4] with the appropriate terms and
numbers.
The rule for how characters and symbols are rep
```
