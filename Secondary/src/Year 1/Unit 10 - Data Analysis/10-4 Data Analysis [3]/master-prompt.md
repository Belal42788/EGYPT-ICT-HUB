...
> **Lesson**: Data Analysis [3] (`10-4`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 134–135 (book numbering)
> **Output filename**: `data-analysis-3.html`
...
Save the site as `data-analysis-3.html` in this same folder.
...
**2. مفهوم Deviation** — core concept (book pages 134–135). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Deviation`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. مفهوم Variance** — core concept (book pages 134–135). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Variance`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**4. مفهوم Standard deviation** — core concept (book pages 134–135). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Standard deviation`. Interaction: bar-chart dashboard from the book's numbers; values hidden then revealed (max/min highlighted); إعادة . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
...
```
Warm Up
The following data represents the results of a handball throw by six individuals.
26,  25,  32,  28,  32,  25  (m)
(1)	 Determine the mean x of the data.
(2)	 Determine the variance and standard deviation of the data.
Explanation
(1)	 Summarize the data in a table like the one on the right.
The mean is 28 m.
(2)	 The variance is 9, and the standard deviation is √ 9  = 3 (m).
3 m
x
x - x
(x - x)2
...
9
135
The following data represents the English vocabulary test scores of ten students.
9,  3,  4,  10,  10,  5,  7,  9,  10,  3  (points)
(1)	 Determine the mean of the data.
(2)	 Determine the variance and standard deviation of the data to two decimal places. However, let √ 2  =
1.414.
The following table shows the number of days it took five students to complete two types of assignments, A and
B.
A
22
28
...
25
28
(1)	 Determine the variance for assignment A.
(2)	 Determine the standard deviation to two decimal places for assignment B. However, let √ 2  = 1.414.
(3)	 Answer which data set, assignment A or assignment B, has a greater variance.
The following data represents the weights of eight harvested strawberries.
22,  25,  18,  17,  22,  21,  20,  15  (g)
(1)	 Determine the mean of the data.
(2)	 Determine the variance and standard deviation of the data.
The following table shows the test scores in math and English for five students.
Math
7
9
6
...
4
2
(1)	 Determine the variance of the math scores.
(2)	 Determine the standard deviation of the English test scores to two decimal places. However, let √ 2  =
1.414.
(3)	 Answer which data set has a greater variance, math or English.
Try
1
2
Exercise
