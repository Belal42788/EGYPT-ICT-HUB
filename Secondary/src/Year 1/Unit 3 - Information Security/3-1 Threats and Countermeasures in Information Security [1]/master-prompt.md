...
> **Lesson**: Threats and Countermeasures in Information Security [1] (`3-1`)
> **Source**: `ICT_En_Sec1_T1.pdf`, printed pages 24–27 (book numbering)
> **Output filename**: `threats-and-countermeasures-in-information-security-1.html`
...
Save the site as `threats-and-countermeasures-in-information-security-1.html` in this same folder.
...
**2. المعلومات (Information security)** — core concept (book pages 24–27). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Information security`. Interaction: threat-vs-defense board: attacker approaches, defense blocks on press; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**3. مفهوم Unauthorized access** — core concept (book pages 24–27). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Unauthorized access`. Interaction: threat-vs-defense board: attacker approaches, defense blocks on press; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**4. مفهوم Cracking** — core concept (book pages 24–27). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Cracking`. Interaction: threat-vs-defense board: attacker approaches, defense blocks on press; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**5. مفهوم Malware** — core concept (book pages 24–27). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Malware`. Interaction: threat-vs-defense board: attacker approaches, defense blocks on press; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
**6. مفهوم Cybercrime** — core concept (book pages 24–27). Lead with the VISUAL (SVG or interactive, never raw `<text>` inside SVG), then a self-contained Egyptian-Arabic paragraph for `Cybercrime`. Interaction: threat-vs-defense board: attacker approaches, defense blocks on press; replay . Close with `.error-box` «أخطاء شائعة» specific to this term. Hidden-click quiz: 1-2 questions (skip button).
...
(1)	 Choose the one in which availability is compromised in terms of information security from the options A
to C below.
A	 A cyberattack caused a website to go down.
B
Incorrect data was entered due to a keyboard typing error.
C	 Personal information was leaked due to a malware infection on the computer.
(2)	 Choose all actions that constitute a violation of the Unauthorized Computer Access Law from the options
A to D below.
A	 Illegally used another person’s user ID and password to access a computer.
B
Stored a password that was obtained illegally on a computer.
...
B
When information is no longer accurate, integrity is compromised.
C When unauthorized individuals can view the information, confidentiality is compromised.
Therefore, the correct answer is A.
(2)	 A	 Using a computer without having access rights constitutes unauthorized access and is prohibited
under the Unauthorized Computer Access Law.
B
Storing a password obtained illegally for the purpose of unauthorized access is prohibited under the
Unauthorized Computer Access Law.
C	 Sharing someone else’s password with a third party without valid reason or permission promotes
unauthorized access and is also prohibited by law.
D	 Publishing illegal content may fall under network-related crimes.
Therefore, the correct answers are A, B, and C.
26
There are three elements of information security: Integrity, Confidentiality, and Availability.  For each of these
elements, choose the one most appropriate measure from the following options A to E.
(1)　Integrity　　　(2)　Confidentiality　　　(3)　Availability
A	 Report your arrival at work to the network administrator.
B
Handle special data such as personal ID numbers in a room where only authorized personnel are allowed to
enter.
C Keep logs of data access and modifications to enable traceability.
D	 Install backup power supplies for all devices related to critical information systems in preparation for
power outages.
E
Run malware in order to always have access t
```
